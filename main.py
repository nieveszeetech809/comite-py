...
def generar_script_migracion():
    ...
    for row in original_rows:
        ...
        if llave in generated_dict:
            ...
            gender_sql = f"'{gender}'" if gender != 'NULL' else "NULL"
            
            sql = f"UPDATE public.\"Participants\" SET foot_size = {foot_sql}, pants_size = {pants_sql} , gender = {gender_sql} WHERE _id = '{_id}';"
            updates.append(sql)
        else:
